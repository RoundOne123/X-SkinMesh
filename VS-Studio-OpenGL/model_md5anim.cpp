...
namespace xMD5Loader
{
	...
	// 加载动画文件
	bool MD5_Animation::LoadAnimation(string &path)
	{
		...
		// 设置默认帧的所有骨骼（在模型空间的位置和方向）数据
		for (int i = 0; i < numJoints; i++)
		{
			X_SkeletonJoint joint;
	...
	// 根据这一帧的数据计算所有骨骼在模型空间中位置和方向
	void MD5_Animation::BuildFrameSkeleton(FrameSkeletonList& skeletons, JointInfoList& jointInfos, BaseFrameList& baseFrames, X_FrameData& frameData)
	{
		// 某一帧的所有骨骼的数据
		X_FrameSkeleton skeleton;
		// 遍历所有原始骨骼数据
		for (unsigned int i = 0; i < jointInfos.size(); i++)
		{
			...
			const X_JointInfo& jointInfo = jointInfos[i];	// 骨骼i原始数据
			// 骨骼i在父骨骼空间的位置和方向数据 初始值为默认baseframe 对应的默认值
			X_SkeletonJoint animatedJoint = baseFrames[i];	
			...
			if (animatedJoint.parentID >= 0)
			{
				...
				*/
				...
				X_SkeletonJoint &parentJoint = skeleton.jointList[animatedJoint.parentID];
				// 这里最终的位置 是一层层计算得到的
	...
	// fInterpolate：插值数
	void MD5_Animation::InterpolateSkeletons(X_FrameSkeleton& finalSkeleton, X_FrameSkeleton& skeleton0, X_FrameSkeleton& skeleton1, float fInterpolate)
	{
		for (int i = 0; i < numJoints; i++)
		{
			X_SkeletonJoint& joint0 = skeleton0.jointList[i];
			X_SkeletonJoint& joint1 = skeleton1.jointList[i];
			X_SkeletonJoint& finalJoint = finalSkeleton.jointList[i];
	...
	// 走帧  主要是插值动画数据
	void MD5_Animation::Update(float deltaTime)
	{
		animTime += deltaTime;
	...
	const MD5_Animation::X_FrameSkeleton& MD5_Animation::GetSkeleton() const
	{
